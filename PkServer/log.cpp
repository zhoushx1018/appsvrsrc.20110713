...
void Log::write_log (int lvl, const char * file, int line, const char* fmt, ...)
{
	...
	if (fp != NULL)
	{
		...
		char log_buf [512];
		localtime_r(&now, &tm);
		int pos = snprintf(log_buf, sizeof(log_buf), "[%d][%04d-%02d-%02d %02d:%02d:%02d]: ",
           		getpid(), tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
	
		va_list ap;
		va_start(ap, fmt);
		pos += vsnprintf(log_buf + pos, sizeof(log_buf) - pos, fmt, ap);
		va_end(ap);

		snprintf(log_buf+ pos, sizeof(log_buf) -pos, " [%s:%d]", file, line);
